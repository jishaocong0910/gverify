...
func TestCheckPredicate(t *testing.T) {
	r := require.New(t)
	{
		// 成功
		vc := vfy.NewDefaultContext()
		vfy.CheckPredicate(vc, ptr(5), nil, 0, nil, func() bool {
			return true
		})
		code, _, _ := vfy.GetResult(vc)
		r.Equal(vfy.SUCCESS, code)
	...
	}
	{
		...
		vc := vfy.NewDefaultContext()
		vfy.SetFieldName(vc, "param")
		vfy.CheckPredicate(vc, (*int)(nil), nil, 0, nil, func() bool {
			return true
		})
		code, _, _ := vfy.GetResult(vc)
		r.Equal(vfy.FAIL, code)
	...
	}
	{
		...
		vc := vfy.NewDefaultContext()
		vfy.SetHasWrong(vc)
		vfy.CheckPredicate(vc, (*int)(nil), []vfy.ItemOption{vfy.Code("MY_CODE")}, 0, nil, func() bool {
			return false
		})
		code, _, _ := vfy.GetResult(vc)
		r.Equal(vfy.FAIL, code)
...
func testNumberToConfine[T vfy.Number](r *require.Assertions, ntcf func(t ...T) []string) {
	confines := ntcf(1, 20, 100)
	r.Len(confines, 3)
