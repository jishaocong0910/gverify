...
func checkRequired[C comparable, A any, T *A | []A | map[C]A](vc *VContext, t T, opts []ItemOption) {
	...
	if t == nil {
		vc.fail(msgBuildFuncRequired, nil)
	}
}

func checkPredicate[C comparable, A any, T *A | []A | map[C]A](vc *VContext, t T, opts []ItemOption, mbf msgBuildFunc, confineFunc func() []string, predicate func() bool) {
	if vc.interrupt() {
		return
	}
	for _, opt := range opts {
